...
```
        ...
        PluginManager              WindowManager
               │                         │
        RuntimeManager              Host (接口)
               │                         │
      ┌────────┴────────┐         ┌──────┴──────┐
      │                 │         │             │
 PluginRuntime    PluginRuntime  LauncherHost  FloatingHost
      │                 │         (主窗口)      (独立窗口)
...
```typescript
class PluginManager {
    scan(): PluginManifest[]; // 扫描 plugins/ 目录
    install(path: string): void; // 安装（复制到 plugins/）
    uninstall(pluginId: string): void; // 卸载
    update(pluginId: string): void; // 更新 → reload Runtime
}
```
...
管理窗口和 Host。**WindowManager 和 HostManager 是同一个东西。**

```typescript
class WindowManager {
    createHost(type: 'launcher' | 'floating'): Host;
    disposeHost(hostId: string): void;
    switchHost(pluginId: string, targetHost: 'launcher' | 'floating'): void;
    // switchHost 用 pluginId 查找 Runtime，渲染进程不感知 runtimeId
}
```
...
整个过程 WebContents 不重建，插件状态、AI Session、滚动位置全部保留。

## 插件更新

PluginManager 检测更新 → 下载 → 挂起当前 Runtime → 替换文件 → reload Runtime（复用同一个 WebContentsView）。

## 搜索去重与排序

不同插件结果按 `group` 分节展示。同一 group 内按 `score` 降序。全局不跨 source 去重。内置系统结果优先级高于插件结果。

## 插件间通信

`utools.redirect(label, payload?)` 跳转到另一个插件的 feature。RuntimeManager 转发。长期可扩展消息总线。

## 多显示器

窗口在鼠标所在屏幕 1/3 处定位。`display-metrics-changed` 时 WindowManager 调整位置和大小。

## 数据持久化

- 应用配置：`~/.szybko/config.json`
- 插件数据：SQLite，每个插件独立 namespace
- 具体 schema 实现阶段按需定义

## 适配器接口

适配器返回**领域类型**（`FileInfo`、`AppInfo` 等），不是 `SearchResult`。`SearchResult` 是 UI 层概念，由插件或宿主系统搜索负责转换。接口定义在 `@szybko/shared` 中，实现过程中逐步补充。
