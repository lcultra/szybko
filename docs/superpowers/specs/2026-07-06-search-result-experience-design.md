...
优化启动器搜索结果链路的用户体验与一致性：搜索无命中时不显示“没有找到匹配结果”，而是回落展示与未输入内容相同的默认页内容（最近使用 + 已固定），同时保留搜索框中的输入。顺带补齐 session 失效、执行上下文绑定、固定/最近结果还原、pin/reorder 刷新、右键菜单统一、前端样式交互走查等问题。
...
8. **右键能力不统一**：结果 item 右键能力不一致；菜单也不应扩展出与本轮无关的操作，只保留固定到“搜索框”和取消固定“搜索框”。
9. **前端样式交互缺少专项验收**：搜索体验调整会影响空态、loading、默认区、fallback 区、键盘执行、点击执行、拖拽排序等 UI 行为，需要单独检查。

## 目标

- 默认页固定返回两个 section，顺序为：最近使用、已固定。
- query 有命中时，只返回“最佳匹配结果”一个 section。
- query 无命中时，最终回落展示默认页内容：最近使用 + 已固定。
...
- 所有结果 item 都支持右键菜单；菜单项只有固定到“搜索框”或取消固定“搜索框”。
...
- **默认内容是一等结果**：Recent/Pinned 不只是空 query 页面，也可作为无命中 fallback。
- **搜索结果保持聚焦**：有 query 且命中时只展示最佳匹配结果，不混入默认 section。
...
搜索框有 query 且 plugin/search provider 有命中时，只展示一个 section：

```
最佳匹配结果
```

该 section 包含当前 query 命中的 item，按 score 排序。默认 section（最近使用、已固定）不与搜索命中结果混排。执行上下文必须绑定本次 query。
...
- query 为空：返回默认内容。
- query 非空且 search section 有 item：只返回“最佳匹配结果”一个 section。
- query 非空且 search section 无 item：返回默认内容作为 final response。

默认内容来自同一次 session 中的 `RecentSectionProvider` 和 `PinnedSectionProvider`，section 顺序固定为最近使用在前、已固定在后。这保证 fallback item 可以被当前 session registry 解析和执行。
...
右键菜单固定行为对所有结果 item 生效，不按 section 或 ownerProvider 分支隐藏。菜单项固定为二选一：

- 未固定：显示 `固定到“搜索框”`，执行 add。
- 已固定：显示 `取消固定“搜索框”`，执行 remove。

本轮不在 item 右键菜单中提供 reveal、preview、删除、打开路径等其他操作。
...
1. **默认页**：空 query 时最近使用、已固定两个 section 顺序正确，section 间距、滚动高度、tile 尺寸不跳动。
2. **无结果 fallback**：输入不存在的 query 后，搜索框保留输入，结果区域展示默认页，无“没有找到匹配结果”文案。
3. **搜索命中页**：有 query 且命中时只出现“最佳匹配结果”一个 section，不混入最近使用或已固定。
4. **loading 过渡**：快速输入时旧内容不闪烁成空态，且不可执行。
5. **键盘交互**：方向键、Enter、Escape 在默认页、搜索结果页、fallback 页行为一致。
6. **鼠标交互**：hover 选中、点击执行、pin 按钮、右键菜单、拖拽排序不互相误触。
7. **右键菜单**：所有结果 item 都可打开右键菜单；菜单只出现固定到“搜索框”或取消固定“搜索框”。
8. **拖拽排序**：拖拽后视觉顺序和刷新后的持久化顺序一致。
9. **固定状态**：同一个 item 在 search/fallback/pinned section 中 pinned 状态一致。
10. **文本与布局**：长标题不撑破 tile，不和 pin icon 重叠；结果区域没有不合理空白或重叠。
...
1. 启动桌面端。
2. 空 query 查看默认页，确认 section 顺序为最近使用、已固定。
3. 输入一个有命中的 query，确认只展示“最佳匹配结果”一个 section。
4. 输入一个不存在的 query，确认回落展示默认页且保留输入。
5. 快速输入多个 query，期间按 Enter 不执行旧结果。
6. 搜索并执行一个 plugin command，确认进入 payload 对应 feature。
7. pin/unpin item，确认 UI 刷新且状态一致。
8. 拖拽 pinned item，确认刷新后顺序稳定。
9. 对默认页、搜索页、fallback 页的 item 打开右键菜单，确认只有固定到“搜索框”或取消固定“搜索框”。
10. 检查样式交互验收清单中的所有项目。
...
- PluginProvider.resolve 需要查询投影表，默认页结果越多，resolve 成本越高。当前 recent/pinned 数量有限，可接受。
- 右键菜单动作发生在 main 侧，刷新依赖 current session snapshot。实现时需要避免在 session 已过期或已取消时重新 emit。

## 验收标准

- query 无命中时不显示“没有找到匹配结果”。
- 空 query 时返回两个默认 section：最近使用、已固定。
- query 有命中时只返回“最佳匹配结果”一个 section。
- query 无命中时展示与空 query 相同的数据来源：最近使用 + 已固定。
- query 无命中时搜索框输入不被清空。
...
- 所有结果 item 支持右键菜单，且菜单只包含固定到“搜索框”或取消固定“搜索框”。
- 前端样式和交互走查通过。
