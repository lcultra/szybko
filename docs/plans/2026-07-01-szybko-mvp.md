...
export default function App() {
    return (
        <div className="flex h-screen w-[820px] items-center justify-center
...
export const Input = forwardRef<HTMLInputElement, InputProps>(
    ({ className = '', ...props }, ref) => {
        return (
            <input
                ref={ref}
                className={`w-full border-none bg-transparent text-2xl
    ...
    return (
        <div
            className="w-[820px] overflow-hidden rounded-[20px] border
                 border-border bg-surface/80 shadow-xl
                 backdrop-blur-xl"
            style={{
...
export function SearchBar({ value, onChange, placeholder = '搜索应用、命令、文件、插件...' }: SearchBarProps) {
    ...
    return (
        <div className="drag flex h-[68px] items-center px-4">
            <Input
                ref={inputRef}
                value={value}
    ...
    return (
        ...
        >
            <div className="flex size-10 items-center justify-center rounded-lg bg-surface-card text-sm">
                {item.icon ? <img src={item.icon} className="size-6" /> : '📄'}
            </div>
...
```typescript
import { MAX_WINDOW_HEIGHT, MIN_WINDOW_HEIGHT } from '@szybko/shared';
import { useEffect, useRef } from 'react';
...
import { handleThemeGet, setupThemeListener } from './theme.js';
...
```typescript
import type { ActionDescriptor, SearchBatch, SearchRequest } from '@szybko/shared';
import { IPC } from '@szybko/shared';
...
```typescript
import type { Host, PluginRuntime, RuntimeState } from '@szybko/shared';
...
```typescript
import { getCore, loadNative } from './adapter-bridge.js';
