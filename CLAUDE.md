...
- `pnpm build` — 构建 TS 包
- `pnpm --filter @szybko/native build` — 构建 Rust 核心
...
- **electron-vite** 负责 main + preload 的 esbuild 打包，renderer 源码位于 `apps/desktop/src/renderer`，preload 分 host.ts（宿主窗口）和 plugin.ts（插件运行时）
